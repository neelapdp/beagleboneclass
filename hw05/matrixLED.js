...
//	socket.emit('i2c', i2cNum);
    ...
    if(disp[i]>>j& 0x1 === 1 & dispRed[i] >> j & 0x1 === 1) {
        //toggle orange to green
    ...
    } else if (disp[i]>>j& 0x1 === 1 | dispRed[i] >> j & 0x1 === 1){
    ...
    */
	...
	for (i = 1; i < data.length; i +=2) {
		...
		//check if both are on (red,green)
		if (($('#id' + i + '_' + j).hasClass('on')) & ($('#id' + i + '_' + j).hasClass('red'))) {
                    $('#id' + i + '_' + j).removeClass('on');
                    $('#id' + i + '_' + j).removeClass('red');
                    $('#id' + i + '_' + j).addClass('orange');
                }
